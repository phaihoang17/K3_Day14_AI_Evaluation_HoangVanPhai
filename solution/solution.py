...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for i, chunk in enumerate(contexts):
            ...
            intersection = len(chunk_tokens & expected_tokens)
            score = intersection / len(expected_tokens)
...
class LLMJudge:
    ...
    def score_response(
        ...
        try:
            ...
            if "scores" in scores and "reasoning" in scores:
                return scores
            return {"scores": {k: 0.5 for k in rubric.keys()}, "reasoning": "Parse error"}
        except Exception:
            return {"scores": {k: 0.5 for k in rubric.keys()}, "reasoning": "Parse error"}
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        rel_sum = sum(r.relevance for r in results)
        comp_sum = sum(r.completeness for r in results)
        ...
        for r in results:
            if r.context_recall is not None:
                recall_sum += r.context_recall
                recall_count += 1
            if r.context_precision is not None:
                prec_sum += r.context_precision
                prec_count += 1
        ...
        return {
            ...
            "avg_faithfulness": faith_sum / total,
            "avg_relevance": rel_sum / total,
            "avg_completeness": comp_sum / total,
            "avg_context_recall": recall_sum / recall_count if recall_count > 0 else None,
            "avg_context_precision": prec_sum / prec_count if prec_count > 0 else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for name, key in metrics:
            if base_report.get(key, 0) - new_report.get(key, 0) > 0.05:
                regressions.append(name)
                
        return {
            "new_avg_faithfulness": new_report.get('avg_faithfulness', 0),
            "new_avg_relevance": new_report.get('avg_relevance', 0),
            "new_avg_completeness": new_report.get('avg_completeness', 0),
            "baseline_avg_faithfulness": base_report.get('avg_faithfulness', 0),
            "baseline_avg_relevance": base_report.get('avg_relevance', 0),
            "baseline_avg_completeness": base_report.get('avg_completeness', 0),
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if sum(1 for v in scores.values() if v == min_score) > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            ...
            sug = suggestions[i] if i < len(suggestions) else (suggestions[-1] if suggestions else "Review required")
            log += f"| {fid} | {ftype} | {rc} | {sug} | Open |\n"
        return log
